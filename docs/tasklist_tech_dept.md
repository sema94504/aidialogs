...
**Задачи:**
- [ ] Добавить mypy в `pyproject.toml`:
  ```toml
  [project.optional-dependencies]
  dev = [
      "pytest>=7.0.0",
...
- [ ] Обновить `Makefile` - добавить команду typecheck:
  ```makefile
  typecheck:
  	uv run mypy src/
  ```

- [ ] Убрать try/except хак с импортами в `src/bot.py`:
  ```python
  # Было:
  try:
...
- [ ] Добавить type hints в `src/bot.py` (если отсутствуют):
  - Уже есть: `user_sessions: dict[int, list[dict]]`
  - Проверить все методы

- [ ] Добавить type hints в `src/llm_client.py` (если отсутствуют):
